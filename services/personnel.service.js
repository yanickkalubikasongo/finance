                        ...
                        if(data.type_membre == 6){
                               ...
                               /* if(await this.check_pseudo_membre(data.prenom)==''){
                                ...
                                let code_us=''
                                 ...
                                 let numPhone= user.dataValues.telephone,
                                    message =`Votre compte vient d'être crée avec succès chez Finuseco pseudo: ${part_membre.dataValues.pseudo} mdp : ${mdp_user} Vous aurez un IM lorsque vous serez affecté à une structure Veuillez changer votre mot de passe pour le sécuriser via https://finuseco.com. Merci.`,
                            ...
                            await sequelize.transaction(async (t) => {
                              /*if(await this.check_pseudo_employe(data.prenom)==''){
                                ...
                                let numPhone= user.dataValues.telephone,
                                    message =`Bonjour, vous êtes désormais membre adhérent chez Finuseco pseudo: ${part_membre.dataValues.pseudo} mdp : ${mdp_user} IM :  ${'0001-'+ code_us} Veuillez changer votre mot de passe pour le sécuriser. Bienvenue dans notre structure et merci de votre confiance.`,
                                    url = `https://api2.dream-digital.info/api/SendSMS?api_id=API12506297241&api_password=x3McB19tzd&sms_type=T&encoding=T&sender_id=FINUSECO&phonenumber=${numPhone}&textmessage=${message}`;
                                    const response = await axios.get(url);
                            })
                        }
